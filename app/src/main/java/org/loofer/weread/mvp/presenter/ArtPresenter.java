...
import org.loofer.weread.mvp.ui.adapter.ArtAdapter;
...
public class ArtPresenter extends BasePresenter<ArtContract.Model, ArtContract.View> {
    ...
    public ArtPresenter(ArtContract.Model model, ArtContract.View rootView
        ...
        mAdapter = new ArtAdapter(mHomeItemList);
        mRootView.setAdapter(mAdapter);//设置Adapter
        mAdapter.setOnItemClickListener(mOnRecyclerViewItemClickListener);
    }


    public void getListByPage(int mode, final boolean pullToRefresh) {
        if (pullToRefresh) {
            this.mPage = 1;
            this.pageId = "0";
            lastItemCreateTime = "0";
        } else {
            this.pageId = mAdapter.getLastItemId();
            this.lastItemCreateTime = mAdapter.getLastItemCreateTime();
        }
        mModel.getListByPage(mPage, mode, pageId, DeviceUtils.getDeviceId(mApplication), lastItemCreateTime)
